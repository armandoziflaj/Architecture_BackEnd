...
| **Database**        | PostgreSQL (or SQL Server)               | A powerful, open-source object-relational database system.                                                |
| **Authentication**  | JWT (JSON Web Tokens)                    | A standard, secure way to handle authentication for APIs.                                                 |
| **Testing**         | xUnit / NUnit                            | The standard testing frameworks for .NET, used for unit and integration testing.                          |
...
-   **Dependency Injection (DI):**
    -   All services and repositories must be registered in `Program.cs` or `Extensions` and injected via constructors.
...
-   **Request Handling:** A request flows from the `Controller` to the `Service`. The controller is responsible for model validation and returning the appropriate HTTP status code.
-   **Error Handling:** A global exception handling middleware (`ExceptionMiddleware`) is used to catch unhandled exceptions and return a standardized error response.
-   **Authentication:** JWT-based authentication is used. The token is passed in the `Authorization` header. An `[Authorize]` attribute should be used on controllers or actions that require authentication.
...
-   **Unit Tests:** All services must have corresponding unit tests. Business logic should be thoroughly tested.
-   **Integration Tests:** Controller endpoints should have integration tests to verify the entire request pipeline, including authentication and validation.
-   **Code Coverage:** A minimum code coverage of 80% is required for all new features.
